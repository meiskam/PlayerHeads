 ...
 */
abstract public class CompatiblePlugin {
     ...
     * @param parentPlugin the current plugin requiring the compatibility (used by child classes for events and logging)
    ...
    public CompatiblePlugin(Plugin parentPlugin, String pluginName) {
        ...
        if(!pluginName.isEmpty()) parentPlugin.getLogger().info(pluginName+" "+(present?"support":"not")+" detected.");
    }

    /**
     * Get the plugin instance for the third-party plugin being supported.
     * This method has Bukkit look-up the plugin name every time it is called, unless the plugin name is blank - you should use get() instead.
     * @return the plugin instance, or null of the plugin name is blank
     */
    protected final Plugin getPlugin() {
        //System.out.println("get pluginName: "+pluginName);
        if(pluginName.isEmpty()) return null;
    ...
    /**
     * Get the plugin instance for the third-party plugin being supported.
     * This method only looks up the plugin name with Bukkit if it was initially detected to be present.
     * @return the plugin instance, or null if the plugin was not present at startup.
     */
    public Plugin get() {
